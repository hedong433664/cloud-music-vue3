<template>
  <div class="song-table_container">
    ...
    <ContextMenu :menu="menuList">
      <div class="table-body">
        <div v-for="(item, index) in songList" :key="item.id" class="row-box" @dblclick="playSong(item)" @contextmenu="handleRowContextmenu(item)">
          <div class="index">
            <PlayerIcon v-show="playState && currentSong.id === item.id" class="player-icon" />

            <span v-show="!playState || currentSong.id !== item.id">{{ index + 1 }}</span>
          </div>
          ...
          <div class="song">
            ...
            />
            <div class="name">
              <span class="song-name" :style="{ color: item.id === currentSong.id ? '#ff3a3a' : ''}" :title="item.name">{{ item.name }}</span>
...
const emits = defineEmits(['play-song'])
